...
namespace PantryToPlate.Models
{
    public class MahlzeitEintrag
    {
        ...
        public MahlzeitEintrag(string name, double menge, double kalorien, double proteine, double kohlen, double fett)
        {
            Name = name;
            Menge = menge;
            Kalorien = kalorien;
            Proteine = proteine;
            Kohlenhydrate = kohlen;
            Fett = fett;
        }
        ...
        public static List<MahlzeitEintrag> LadeVonTag(DateTime datum, string dateiPfad = "data/tagesmahlzeiten.csv")
        {
            ...
            for (int i = 1; i < zeilen.Length; i++)
            {
                ...
                if (double.TryParse(teile[2], NumberStyles.Any, CultureInfo.InvariantCulture, out double menge) &&
                    double.TryParse(teile[3], NumberStyles.Any, CultureInfo.InvariantCulture, out double kal) &&
                    double.TryParse(teile[4], NumberStyles.Any, CultureInfo.InvariantCulture, out double pro) &&
                    double.TryParse(teile[5], NumberStyles.Any, CultureInfo.InvariantCulture, out double kohlen) &&
                    double.TryParse(teile[6], NumberStyles.Any, CultureInfo.InvariantCulture, out double fett))
                {
                    result.Add(new MahlzeitEintrag(teile[1], menge, kal, pro, kohlen, fett));
                }
            }
            return result;
        }
